...
import { deedSearchApi, deedSearchUi, lrArchiveApi, lrArchiveUi } from './projects';
...
(async () => {
    ...
    await db.delete(schema.resourceLink);
    ...
    await db.insert(schema.technology).values([reactTech(), angularTech(), nestJsTech()]);
    ...
    await db.insert(schema.project).values([deedSearchApi(), deedSearchUi(), lrArchiveApi(), lrArchiveUi()]);
