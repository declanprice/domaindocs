...
export const resourceLink = pgTable(
    ...
    (table) => {
        return {
            subdomainIndex: index('resource_link_subdomain_index').on(table.subdomainId),
            projectIndex: index('resource_link_project_index').on(table.projectId),
        };
    },
);

export const resourceLinkRelations = relations(resourceLink, ({ one }) => ({
