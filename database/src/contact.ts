...
export const contact = pgTable(
    ...
    (table) => {
        return {
            ...
            subdomainIndex: index('contact_subdomain_index').on(table.subdomainId),
            projectIndex: index('contact_project_index').on(table.projectId),
            personSubdomainUnique: uniqueIndex('contact_person_subdomain_unique').on(table.subdomainId, table.personId),
            personProjectUnique: uniqueIndex('contact_person_project_unique').on(table.projectId, table.personId),
        };
    },
);
